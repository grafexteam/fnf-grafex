...
class ColorblindFilters {
    ...
    public static function applyFiltersOnGame() {
        ...
        if (ClientPrefs.ColorBlindType != "None") { // actually self explanatory, isn't it?
            if (filterMap.get(ClientPrefs.ColorBlindType) != null) { // anticrash system
                var thisF = filterMap.get(ClientPrefs.ColorBlindType).filter;
                if (thisF != null) {
