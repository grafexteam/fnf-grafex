## Building FNF: Graphex Engine.

## **NOTES**
- You should know how to work with Command Line. 
- If you want to build for *Windows*, you must be on *Windows*. If for *Linux* - on *Linux* and etc.
- If you just want to play this engine, not code it, you can look for .ZIP archive in the [releases tab](https://github.com/XaleTheCat/fnf-grafex/releases)
...
### Windows-only dependencies (only for building *to* Windows.)
If you are planning to build for Windows, you also need to install **Visual Studio 2019**. While installing it, *don't click on any of the options to install workloads*. Instead, go to the **individual components** tab and choose the following:

-   MSVC v142 - VS 2019 C++ x64/x86 build tools
-   Windows SDK (10.0.17763.0)

This will install about 4 GB of c++ stuff, but is necessary to build for Windows.
...
2. `git clone https://github.com/XaleTheCat/fnf-grafex.git`
3. `cd` into the source code: `cd fnf-graphex`
...
- The build will be in `fnf-graphex/export/release/<target>/bin`, with `<target>` being the target you built to in the previous step. (i.e. `fnf-graphex/export/release/windows/bin`)
- Incase you added the -debug flag the files will be inside `fnf-graphex/export/debug/<target>/bin`
- Only the `bin` folder is necessary to run the game. The other ones in `export/release/<target>` are not.

### This was taken from Kade Engine docs cuz YEAH :)
